...
class LandsatProcessingTestCase(ActiniaResourceTestCaseBase):
    def test_landsat_computation_DOS4(self):
        rv = self.server.post(
            URL_PREFIX + "/landsat_process/LT41970251990147XXX03/DOS4/NDVI",
    ...
    def test_landsat_computation_TOAR(self):
        rv = self.server.post(
            URL_PREFIX + "/landsat_process/LT41970251990147XXX03/TOAR/NDVI",
            headers=self.admin_auth_header,
        )
        pprint(json_load(rv.data))
        self.assertEqual(
    ...
    def test_landsat_computation_error_1(self):
        rv = self.server.post(
            URL_PREFIX + "/landsat_process/LT41970251990147XXX03/POS/NDVI",
            headers=self.admin_auth_header,
        )
        pprint(json_load(rv.data))
        self.assertEqual(
    ...
    def test_landsat_computation_error_2(self):
        rv = self.server.post(
            URL_PREFIX + "/landsat_process/LT41970251990147XXX03/TOAR/LOLO",
            headers=self.admin_auth_header,
        )
        pprint(json_load(rv.data))
        self.assertEqual(
    ...
    def test_landsat_computation_error_3(self):
        rv = self.server.post(
            URL_PREFIX
            + "/landsat_process/LT41970241987299XXX0_NOPE/TOAR/NDVI",
            headers=self.admin_auth_header,
        )
        pprint(json_load(rv.data))
        self.assertEqual(
