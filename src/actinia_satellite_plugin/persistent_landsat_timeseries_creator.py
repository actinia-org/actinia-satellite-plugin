...
class LandsatTimeSeriesCreator(PersistentProcessing):
    ...
    def _prepare_download(self):
        ...
        for scene_id in self.scene_ids:
            ...
            if len(sensor_ids) > 2:
                raise AsyncProcessError("Different satellites are in the list of scenes to be imported. "
        ...
        try:
            self.query_result = self.query_interface.get_landsat_urls(self.scene_ids, self.required_bands)
    ...
    def _import_scenes(self):
        ...
        for scene_id in self.query_result:
            ...
            atcor_method_commands = process_lib.get_i_landsat_toar_process_list(atcor_method=self.atcor_method)
        ...
        for i in range(len(self.required_bands)):
            ...
            map_list_file_name = os.path.join(self.user_download_cache_path, strds)
            ...
            for scene_id in self.query_result:
                ...
                start_time = dtparser.parse( self.query_result[scene_id]["timestamp"].split(".")[0])
                end_time = start_time + timedelta(seconds=1)

                index = SCENE_BANDS[self.sensor_id].index(band)
                raster_suffix = RASTER_SUFFIXES[self.sensor_id][index]
                map_name = "%s_%s%s"%(scene_id, self.atcor_method, raster_suffix)

                result_dict[band].append([map_name, str(start_time), str(end_time)])

                map_list_file.write("%s|%s|%s\n"%(map_name, str(start_time), str(end_time)))
            map_list_file.close()

            register_maps = {"module":"t.register",
                             "inputs":{"input":strds,
        ...
        process_list = self._validate_process_chain(process_chain=register_commands,
    ...
    def _execute(self):
        ...
        if self.target_mapset_exists is True:
            raise AsyncProcessError("Sentinel time series can only be create in a new mapset. "
